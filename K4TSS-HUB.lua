...
local function StartFly()
    ...
    flyThread = task.spawn(function()
        ...
        while flyActive do
            ...
            -- ── Mobile: Analog Joystick (MoveDirection) ──
            -- MoveDirection adalah Vector3 dari input analog (-1 s/d 1 di X dan Z)
            if hum2 then
                local moveDir = hum2.MoveDirection
                if moveDir.Magnitude > 0.1 then
                    -- Proyeksikan MoveDirection ke bidang kamera (horizontal)
                    ...
                    -- Decompose world MoveDirection ke sumbu kamera
                    local fwd   = flatLook:Dot(moveDir)
                    local right = flatRight:Dot(moveDir)
                    dir = dir + (flatLook * fwd) + (flatRight * right)
                end
            end

            -- ── Naik/turun via jump button (mobile) atau Space ──
            ...
            -- Arahkan karakter ke arah kamera (horizontal saja agar tidak miring)
            local flatLook = Vector3.new(camCF.LookVector.X, 0, camCF.LookVector.Z)
            if flatLook.Magnitude > 0.01 then
                flyBodyGyro.CFrame = CFrame.new(hrp2.Position, hrp2.Position + flatLook)
            end
