...
-- Pastikan karakter sudah ada sebelum lanjut (penting agar GetHRP() tidak nil di awal)
...
-- Coba parent ke PlayerGui, fallback ke CoreGui jika gagal
...
local function MakeSmallBtn(parent, text, bgColor, xPos, anchorPt, w, h)
    local btn = Instance.new("TextButton", parent)
    btn.Text = text; btn.Size = UDim2.new(0,w or 44,0,h or 30)
    btn.Position = xPos or UDim2.new(0,0,0,0)
    btn.AnchorPoint = anchorPt or Vector2.new(0,0)
...
-- ─── INFINITE JUMP ───
local ijRow = MakeRow(SpeedTab, 3, 44)
...
-- ─── PRESET BUTTONS ───
local presetRow = MakeRow(SpeedTab, 4, 36)
...
-- ─── RESET ALL ───
local resetAllRow = MakeRow(SpeedTab, 5, 34)
...
-- ─── FLY TOGGLE ───
local flyRow = MakeRow(SpeedTab, 6, 44)
...
local flyThread   = nil
local flyBV       = nil   -- BodyVelocity (fallback lama)
local flyBG       = nil   -- BodyGyro     (fallback lama)
local flyLV       = nil   -- LinearVelocity (Roblox baru)
local flyAT       = nil   -- AlignOrientation (Roblox baru)
local flyAttach0  = nil
local flyAttach1  = nil
local useNewAPI   = (pcall(function() return Instance.new("LinearVelocity") end))

local function StopFly()
    flyActive = false
    if flyThread then task.cancel(flyThread); flyThread = nil end
    -- Hapus instance lama/baru
    ...
    pcall(function() if flyLV  and flyLV.Parent  then flyLV:Destroy()  end end)
    pcall(function() if flyAT  and flyAT.Parent  then flyAT:Destroy()  end end)
    pcall(function() if flyAttach0 and flyAttach0.Parent then flyAttach0:Destroy() end end)
    pcall(function() if flyAttach1 and flyAttach1.Parent then flyAttach1:Destroy() end end)
    flyBV = nil; flyBG = nil; flyLV = nil; flyAT = nil; flyAttach0 = nil; flyAttach1 = nil
    local hum = GetHumanoid()
...
local function StartFly()
    ...
    -- Coba pakai LinearVelocity (API baru), fallback ke BodyVelocity (API lama)
    local ok = pcall(function()
        flyAttach0 = Instance.new("Attachment", hrp)
        flyAttach1 = Instance.new("Attachment", workspace.Terrain)

        flyLV = Instance.new("LinearVelocity", hrp)
        flyLV.Attachment0    = flyAttach0
        flyLV.MaxForce       = 1e5
        ...
        flyAT = Instance.new("AlignOrientation", hrp)
        flyAT.Attachment0    = flyAttach0
        flyAT.MaxTorque      = 1e5
        flyAT.MaxAngularVelocity = 1e5
        flyAT.Responsiveness = 200
        flyAT.RigidityEnabled = true
        flyAT.CFrame         = hrp.CFrame
        flyAT.Mode           = Enum.OrientationAlignmentMode.OneAttachment
    end)

    if not ok then
        -- Fallback: BodyVelocity + BodyGyro
        pcall(function()
            if flyAttach0 then flyAttach0:Destroy() end
            if flyAttach1 then flyAttach1:Destroy() end
        end)
        flyAttach0 = nil; flyAttach1 = nil; flyLV = nil; flyAT = nil
    ...
    flyThread = task.spawn(function()
        ...
        while flyActive do
            ...
            -- Update velocity (new API)
            if flyLV and flyLV.Parent then
                flyLV.VectorVelocity = vel
            end
            -- Update velocity (old API fallback)
            if flyBV and flyBV.Parent then
                flyBV.Velocity = vel
            end

            -- Arahkan karakter ke kamera
            local targetCF = CFrame.new(hrp2.Position, hrp2.Position + camCF.LookVector)
            if flyAT and flyAT.Parent then
                flyAT.CFrame = targetCF
            end
            if flyBG and flyBG.Parent then
                flyBG.CFrame = targetCF
            end
...
-- ════════════════════════════════════════
--    WORLD TAB (Time & Lighting)
-- ════════════════════════════════════════
local WorldTab = NewTab("World", 4)

local Lighting = game:GetService("Lighting")

-- Header
local wldHeaderRow = MakeRow(WorldTab, 1, 40)
wldHeaderRow.BackgroundColor3 = Color3.fromRGB(28,28,28)
local wldHeaderLabel = Instance.new("TextLabel", wldHeaderRow)
wldHeaderLabel.Text = "🌍  Time & Lighting"
wldHeaderLabel.Size = UDim2.new(1,-10,1,0); wldHeaderLabel.Position = UDim2.new(0,10,0,0)
wldHeaderLabel.BackgroundTransparency = 1; wldHeaderLabel.TextColor3 = TEXTW
wldHeaderLabel.Font = Enum.Font.GothamBold; wldHeaderLabel.TextSize = 11
wldHeaderLabel.TextXAlignment = Enum.TextXAlignment.Left

-- ─── TIME OF DAY ───
local timeRow = MakeRow(WorldTab, 2, 54)
local timeLbl = Instance.new("TextLabel", timeRow)
timeLbl.Text = "🕐  Time of Day"
timeLbl.Size = UDim2.new(0,110,0,18); timeLbl.Position = UDim2.new(0,10,0,5)
timeLbl.BackgroundTransparency = 1; timeLbl.TextColor3 = TEXTW
timeLbl.Font = Enum.Font.GothamSemibold; timeLbl.TextSize = 11
timeLbl.TextXAlignment = Enum.TextXAlignment.Left

local timeValLbl = Instance.new("TextLabel", timeRow)
timeValLbl.Text = "12:00"
timeValLbl.Size = UDim2.new(0,50,0,18); timeValLbl.Position = UDim2.new(0,118,0,5)
timeValLbl.BackgroundTransparency = 1; timeValLbl.TextColor3 = GOLD
timeValLbl.Font = Enum.Font.GothamBold; timeValLbl.TextSize = 12
timeValLbl.TextXAlignment = Enum.TextXAlignment.Left

-- Slider waktu (0-24 jam)
local timeTrack = Instance.new("Frame", timeRow)
timeTrack.Size = UDim2.new(1,-24,0,6); timeTrack.Position = UDim2.new(0,12,0,36)
timeTrack.BackgroundColor3 = Color3.fromRGB(50,50,50); timeTrack.BorderSizePixel = 0
Instance.new("UICorner", timeTrack).CornerRadius = UDim.new(1,0)

-- Gradient siang-malam di track
local timeGrad = Instance.new("UIGradient", timeTrack)
timeGrad.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0,    Color3.fromRGB(20,20,60)),    -- tengah malam
    ColorSequenceKeypoint.new(0.25, Color3.fromRGB(255,160,60)),  -- subuh
    ColorSequenceKeypoint.new(0.5,  Color3.fromRGB(120,180,255)), -- siang
    ColorSequenceKeypoint.new(0.75, Color3.fromRGB(255,120,40)),  -- sore
    ColorSequenceKeypoint.new(1,    Color3.fromRGB(20,20,60)),    -- malam
})

local timeFill = Instance.new("Frame", timeTrack)
timeFill.Size = UDim2.new(0.5,0,1,0)
timeFill.BackgroundTransparency = 1; timeFill.BorderSizePixel = 0

local timeThumb = Instance.new("TextButton", timeTrack)
timeThumb.Size = UDim2.new(0,16,0,16); timeThumb.AnchorPoint = Vector2.new(0.5,0.5)
timeThumb.Position = UDim2.new(0.5,0,0.5,0)
timeThumb.BackgroundColor3 = TEXTW; timeThumb.Text = ""; timeThumb.BorderSizePixel = 0; timeThumb.ZIndex = 10
Instance.new("UICorner", timeThumb).CornerRadius = UDim.new(1,0)

local function SetTimeOfDay(hour)
    hour = math.clamp(hour, 0, 24)
    Lighting.TimeOfDay = string.format("%02d:%02d:%02d", math.floor(hour), math.floor((hour % 1) * 60), 0)
    local h = math.floor(hour)
    local m = math.floor((hour % 1) * 60)
    timeValLbl.Text = string.format("%02d:%02d", h, m)
    local rel = hour / 24
    timeThumb.Position = UDim2.new(rel,0,0.5,0)
end

local timeDragging = false
timeThumb.MouseButton1Down:Connect(function() timeDragging = true end)
timeThumb.InputBegan:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.Touch then timeDragging = true end
end)
UserInputService.InputEnded:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.MouseButton1
    or i.UserInputType == Enum.UserInputType.Touch then timeDragging = false end
end)
UserInputService.InputChanged:Connect(function(i)
    if not timeDragging then return end
    if i.UserInputType ~= Enum.UserInputType.MouseMovement
    and i.UserInputType ~= Enum.UserInputType.Touch then return end
    local rel = math.clamp((i.Position.X - timeTrack.AbsolutePosition.X) / timeTrack.AbsoluteSize.X, 0, 1)
    SetTimeOfDay(rel * 24)
end)

-- Time preset buttons
local timePresetRow = MakeRow(WorldTab, 3, 34)
timePresetRow.BackgroundColor3 = Color3.fromRGB(24,24,24)
local tpLbl = Instance.new("TextLabel", timePresetRow)
tpLbl.Text = "Preset:"; tpLbl.Size = UDim2.new(0,46,1,0); tpLbl.Position = UDim2.new(0,10,0,0)
tpLbl.BackgroundTransparency = 1; tpLbl.TextColor3 = TEXTG
tpLbl.Font = Enum.Font.Gotham; tpLbl.TextSize = 10; tpLbl.TextXAlignment = Enum.TextXAlignment.Left

local function MakeTimeBtn(label, hour, xOff, color)
    local btn = MakeSmallBtn(timePresetRow, label, color, UDim2.new(0,xOff,0.5,-11), nil, 52, 22)
    btn.MouseButton1Click:Connect(function() SetTimeOfDay(hour) end)
end
MakeTimeBtn("🌙 Mlm",   0,  58, Color3.fromRGB(30,30,80))
MakeTimeBtn("🌅 Subuh", 6,  114, Color3.fromRGB(180,90,30))
MakeTimeBtn("☀ Siang", 12, 170, Color3.fromRGB(180,140,20))
MakeTimeBtn("🌆 Sore",  17, 226, Color3.fromRGB(180,70,20))
MakeTimeBtn("🌃 Malam", 20, 282, Color3.fromRGB(20,20,80))

-- ─── BRIGHTNESS ───
local brightRow = MakeRow(WorldTab, 4, 54)
local brightLbl = Instance.new("TextLabel", brightRow)
brightLbl.Text = "💡  Brightness"
brightLbl.Size = UDim2.new(0,110,0,18); brightLbl.Position = UDim2.new(0,10,0,5)
brightLbl.BackgroundTransparency = 1; brightLbl.TextColor3 = TEXTW
brightLbl.Font = Enum.Font.GothamSemibold; brightLbl.TextSize = 11
brightLbl.TextXAlignment = Enum.TextXAlignment.Left

local brightValLbl = Instance.new("TextLabel", brightRow)
brightValLbl.Text = "2"
brightValLbl.Size = UDim2.new(0,40,0,18); brightValLbl.Position = UDim2.new(0,118,0,5)
brightValLbl.BackgroundTransparency = 1; brightValLbl.TextColor3 = GOLD
brightValLbl.Font = Enum.Font.GothamBold; brightValLbl.TextSize = 12
brightValLbl.TextXAlignment = Enum.TextXAlignment.Left

local brightTrack = Instance.new("Frame", brightRow)
brightTrack.Size = UDim2.new(1,-24,0,6); brightTrack.Position = UDim2.new(0,12,0,36)
brightTrack.BackgroundColor3 = Color3.fromRGB(50,50,50); brightTrack.BorderSizePixel = 0
Instance.new("UICorner", brightTrack).CornerRadius = UDim.new(1,0)

local brightFill = Instance.new("Frame", brightTrack)
brightFill.Size = UDim2.new(2/10,0,1,0)
brightFill.BackgroundColor3 = GOLD; brightFill.BorderSizePixel = 0
Instance.new("UICorner", brightFill).CornerRadius = UDim.new(1,0)

local brightThumb = Instance.new("TextButton", brightTrack)
brightThumb.Size = UDim2.new(0,16,0,16); brightThumb.AnchorPoint = Vector2.new(0.5,0.5)
brightThumb.Position = UDim2.new(2/10,0,0.5,0)
brightThumb.BackgroundColor3 = TEXTW; brightThumb.Text = ""; brightThumb.BorderSizePixel = 0; brightThumb.ZIndex = 10
Instance.new("UICorner", brightThumb).CornerRadius = UDim.new(1,0)

local brightDragging = false
brightThumb.MouseButton1Down:Connect(function() brightDragging = true end)
brightThumb.InputBegan:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.Touch then brightDragging = true end
end)
UserInputService.InputEnded:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.MouseButton1
    or i.UserInputType == Enum.UserInputType.Touch then brightDragging = false end
end)
UserInputService.InputChanged:Connect(function(i)
    if not brightDragging then return end
    if i.UserInputType ~= Enum.UserInputType.MouseMovement
    and i.UserInputType ~= Enum.UserInputType.Touch then return end
    local rel = math.clamp((i.Position.X - brightTrack.AbsolutePosition.X) / brightTrack.AbsoluteSize.X, 0, 1)
    local val = math.floor(rel * 10 * 10) / 10
    brightFill.Size = UDim2.new(rel,0,1,0); brightThumb.Position = UDim2.new(rel,0,0.5,0)
    brightValLbl.Text = tostring(val)
    Lighting.Brightness = val
end)

-- ─── FOG ───
local fogRow = MakeRow(WorldTab, 5, 54)
local fogLbl = Instance.new("TextLabel", fogRow)
fogLbl.Text = "🌫  Fog"
fogLbl.Size = UDim2.new(0,110,0,18); fogLbl.Position = UDim2.new(0,10,0,5)
fogLbl.BackgroundTransparency = 1; fogLbl.TextColor3 = TEXTW
fogLbl.Font = Enum.Font.GothamSemibold; fogLbl.TextSize = 11
fogLbl.TextXAlignment = Enum.TextXAlignment.Left

local fogValLbl = Instance.new("TextLabel", fogRow)
fogValLbl.Text = "100k"
fogValLbl.Size = UDim2.new(0,50,0,18); fogValLbl.Position = UDim2.new(0,118,0,5)
fogValLbl.BackgroundTransparency = 1; fogValLbl.TextColor3 = GOLD
fogValLbl.Font = Enum.Font.GothamBold; fogValLbl.TextSize = 12
fogValLbl.TextXAlignment = Enum.TextXAlignment.Left

local fogTrack = Instance.new("Frame", fogRow)
fogTrack.Size = UDim2.new(1,-24,0,6); fogTrack.Position = UDim2.new(0,12,0,36)
fogTrack.BackgroundColor3 = Color3.fromRGB(50,50,50); fogTrack.BorderSizePixel = 0
Instance.new("UICorner", fogTrack).CornerRadius = UDim.new(1,0)

local fogFill = Instance.new("Frame", fogTrack)
fogFill.Size = UDim2.new(1,0,1,0)
fogFill.BackgroundColor3 = Color3.fromRGB(150,200,220); fogFill.BorderSizePixel = 0
Instance.new("UICorner", fogFill).CornerRadius = UDim.new(1,0)

local fogThumb = Instance.new("TextButton", fogTrack)
fogThumb.Size = UDim2.new(0,16,0,16); fogThumb.AnchorPoint = Vector2.new(0.5,0.5)
fogThumb.Position = UDim2.new(1,0,0.5,0)
fogThumb.BackgroundColor3 = TEXTW; fogThumb.Text = ""; fogThumb.BorderSizePixel = 0; fogThumb.ZIndex = 10
Instance.new("UICorner", fogThumb).CornerRadius = UDim.new(1,0)

-- Fog end: slider kiri = kabut tebal (dekat), kanan = tidak ada kabut (jauh)
local function FormatFog(val)
    if val >= 1000 then return math.floor(val/1000).."k" else return tostring(math.floor(val)) end
end

local fogDragging = false
fogThumb.MouseButton1Down:Connect(function() fogDragging = true end)
fogThumb.InputBegan:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.Touch then fogDragging = true end
end)
UserInputService.InputEnded:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.MouseButton1
    or i.UserInputType == Enum.UserInputType.Touch then fogDragging = false end
end)
UserInputService.InputChanged:Connect(function(i)
    if not fogDragging then return end
    if i.UserInputType ~= Enum.UserInputType.MouseMovement
    and i.UserInputType ~= Enum.UserInputType.Touch then return end
    local rel = math.clamp((i.Position.X - fogTrack.AbsolutePosition.X) / fogTrack.AbsoluteSize.X, 0, 1)
    -- rel 0 = 50 (kabut sangat tebal), rel 1 = 100000 (tidak ada kabut)
    local val = math.floor(50 + (rel ^ 2) * 99950)
    fogFill.Size = UDim2.new(rel,0,1,0); fogThumb.Position = UDim2.new(rel,0,0.5,0)
    fogValLbl.Text = FormatFog(val)
    Lighting.FogEnd = val
end)

-- ─── RESET LIGHTING ───
local wldResetRow = MakeRow(WorldTab, 6, 34)
wldResetRow.BackgroundColor3 = Color3.fromRGB(26,26,26)
local wldResetBtn = MakeSmallBtn(wldResetRow, "🔄 Reset Lighting ke Default", Color3.fromRGB(55,55,55), UDim2.new(0.5,-85,0.5,-11), nil, 170, 22)
wldResetBtn.MouseButton1Click:Connect(function()
    SetTimeOfDay(14)
    Lighting.Brightness = 2
    Lighting.FogEnd     = 100000
    brightValLbl.Text = "2"
    brightFill.Size = UDim2.new(2/10,0,1,0); brightThumb.Position = UDim2.new(2/10,0,0.5,0)
    fogValLbl.Text = "100k"
    fogFill.Size = UDim2.new(1,0,1,0); fogThumb.Position = UDim2.new(1,0,0.5,0)
end)

-- ════════════════════════════════════════
--    SETTINGS TAB
-- ════════════════════════════════════════
local SettingsTab = NewTab("Settings", 5)
