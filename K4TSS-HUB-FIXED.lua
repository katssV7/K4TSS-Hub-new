...
local StarterGui; pcall(function() StarterGui = game:GetService("StarterGui") end)

local LocalPlayer = Players.LocalPlayer

-- Pastikan karakter sudah ada sebelum lanjut
if not LocalPlayer.Character then
    LocalPlayer.CharacterAdded:Wait()
end

-- ════════════════════════════════════════
--    PURE LUA JSON (tanpa HttpService)
-- ════════════════════════════════════════
...
local function DecodeJSON(str)
    ...
    if hs then
        local ok, res = pcall(function() return hs:JSONDecode(str) end)
...
if not _ok then
    warn("[K4TSS ERROR] " .. tostring(_err))
    -- Tampilkan error sebagai notifikasi in-game
    task.spawn(function()
        task.wait(1)
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title    = "K4TSS ERROR",
                Text     = tostring(_err):sub(1, 80),
                Duration = 15,
            })
        end)
        -- Fallback: buat GUI error sederhana
        pcall(function()
            ...
            local ok2 = false
            pcall(function() sg.Parent = game:GetService("Players").LocalPlayer.PlayerGui; ok2 = true end)
            if not ok2 then pcall(function() sg.Parent = game:GetService("CoreGui") end) end
            local f = Instance.new("Frame", sg)
            f.Size = UDim2.new(0,360,0,80)
            f.Position = UDim2.new(0.5,-180,1,-100)
